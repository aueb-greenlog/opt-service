...
if __name__ == "__main__":
	...
	file = open(f'./instances/optInputDaily.json', encoding='utf-8')
	...
	if areValid:
		uuid, areValid, routeDate, nOrders, nLocations, nDepots, nHubs, nMobile, nLastmile, orders_info, orders_time, orders_size, locations_info, locations_time, vehicles_info, vehicles_size, travelTimes, canBeTransferredWith, mode, event, objective, user, eligible_types, allOperators, livingLab = opt_preprocessing.import_data(input_data)
		if areValid:
			routes, subroutes, maxOrders = opt_orchestrator.optimizer(event, livingLab, nOrders, nLocations, nMobile, orders_info, orders_time, orders_size, locations_info, locations_time, vehicles_info, vehicles_size, travelTimes, canBeTransferredWith, objective)
			if routes != False:	
				output_data, routes = opt_export.export_json(livingLab, vehicles_info, routes, subroutes, orders_info, orders_time, locations_info, travelTimes, uuid, user, maxOrders)
				output_data['general']['uuid'] = uuid
				output_data['general']['userName'] = user
				output_data['general']['producedAt'] = int(time.time() * 1000)
